...
import { CommentCreateData, Comment } from "../types/comment";
...
export function useComments(postId: string){
  const { data: comments = [], ...queryResult } = useQuery<Comment[]>({
...
export function useCommentMutations(postId: string) {
    ...
    const createCommentMutation = useMutation({
      mutationFn: async (newComment: CommentCreateData) => await createComment(newComment),
      onSuccess: () => {
